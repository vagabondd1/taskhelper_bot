...
Отдельный образ (`Dockerfile.test`) — прод-образ ставит только runtime-зависимости, а `tests/` исключён из его контекста через `.dockerignore`. Код монтируется томом, поэтому после правки теста пересобирать образ не нужно; пересборка требуется только при изменении `requirements-dev.txt`.

Инфраструктура для прогона не нужна: ни Postgres, ни Redis, ни ключ от LLM — всё внешнее замокано, тесты гоняются офлайн.

| Файл | Что проверяет |
|---|---|
| `test_llm_contracts.py` | Схема `LLMResponse`: обязательные поля, enum'ы, дефолты, разбор сырого ответа модели |
| `test_plan_sanitizer.py` | Санитайзер плана: границы 3–7 шагов, дубли, обрезка, отказ от мусорного плана |
| `test_session_repo.py` | Мутации состояния сессии: инкремент шага, сброс `awaiting_hypothesis`, `last_*`-поля |
| `test_guided_service.py` | Guided-режим: гварды, конец плана, fallback при сбое LLM, смена режима |
| `test_task_service.py` | Новая задача: cooldown, off-topic, сохранение плана и summary |

Тестовый прогон — только логика. Схема БД, миграции и реальные запросы им не покрыты: это зона `docker compose up`.
